...
### 3. Wrking with Shiny : plot of alcohol consumption in a specific year across Story County + in specific city from 2012 to 2019(another version)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Input: select year for plotting bar chart of alcohol consumption across Story County
      selectInput("Year", label = ("Select Year"),
                  choices = sort(unique(sub_data$Year)),
                  selected = 2012),
      # Input: select city for plotting bar chart of alcohol consumption of a specific city from 2012 to 2019
    ...
    # Main panel for displaying three plot outputs 
    mainPanel(
      tabsetPanel(
...
server <- function(input, output) {
  #filtering data in specific year
  liq_subset <- reactive({
    sub_data %>%
      filter(Year == input$Year)
  })
  #filtering data for specific city
  liqcity_subset <- reactive({
    sub_data %>%
      filter(city == input$city)
  })
  #plot the alcohol consumption in specific year across Story County 
  ...
  # plot the alcohol consumption in specific city from 2012 to 2019 color and group based on the day of week 
